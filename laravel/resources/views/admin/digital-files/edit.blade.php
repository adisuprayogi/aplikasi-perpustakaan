...
<div class="mb-8">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit File Digital</h1>
            <p class="text-sm text-gray-500 mt-0.5">Edit informasi file digital</p>
        </div>
    </div>
</div>

<div class="max-w-3xl">
    <form method="POST" action="{{ route('digital-files.update', $digitalFile) }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-5">
            ...
            <select name="collection_id" id="collection_id" required
                ...
                <option value="{{ $c->id }}" {{ $digitalFile->collection_id == $c->id ? 'selected' : '' }}>{{ $c->title }}</option>
                @endforeach
            </select>
            @error('collection_id')
        ...
        <div class="mb-5">
            ...
            <input type="text" name="title" id="title" value="{{ old('title', $digitalFile->title) }}" required
            ...
            <div class="p-4 bg-gray-50 rounded-xl mb-3">
                <div class="flex items-center">
                    ...
                    <div class="ml-3 flex-1">
                        <p class="text-sm font-medium text-gray-900">{{ $digitalFile->file_name }}</p>
                        <p class="text-xs text-gray-500">{{ $digitalFile->file_size_human }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="text-center">
                    ...
                    <div class="mt-2">
                        <label for="file" class="cursor-pointer">
                            ...
                            <input type="file" name="file" id="file" class="sr-only" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.jpg,.jpeg,.png,.gif">
        ...
        <div class="mb-5">
            ...
            <select name="access_level" id="access_level" required
                ...
                <option value="public" {{ old('access_level', $digitalFile->access_level) == 'public' ? 'selected' : '' }}>Publik (Semua orang)</option>
                <option value="registered" {{ old('access_level', $digitalFile->access_level) == 'registered' ? 'selected' : '' }}>Terdaftar (Login required)</option>
                <option value="campus_only" {{ old('access_level', $digitalFile->access_level) == 'campus_only' ? 'selected' : '' }}>Kampus Saja (Member only)</option>
            </select>
            @error('access_level')
            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-5">
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600 transition"
                placeholder="Deskripsi singkat tentang file (opsional)">{{ old('description', $digitalFile->description) }}</textarea>
            @error('description')
            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-5">
            ...
            <input type="text" name="version" id="version" value="{{ old('version', $digitalFile->version) }}"
                class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600 transition"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
            <div>
                ...
                <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at', $digitalFile->published_at?->format('Y-m-d\TH:i')) }}"
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600 transition">
                @error('published_at')
                <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="flex items-center h-[46px]">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $digitalFile->is_active) ? 'checked' : '' }} class="sr-only peer">
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
            <a href="{{ route('digital-files.show', $digitalFile) }}" class="px-5 py-2.5 text-gray-700 hover:text-gray-900 font-medium rounded-xl hover:bg-gray-100 transition">
                Batal
            </a>
