...
<div class="mb-6">
    <div class="flex items-center justify-between mb-4">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Buat branch perpustakaan baru</p>
        </div>
        <a href="{{ route('branches.index') }}" class="text-blue-700 hover:text-blue-900">
...
<div class="bg-white shadow rounded-lg">
    <form method="POST" action="{{ route('branches.store') }}" class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select id="type" name="type" required
                    ...
                    <option value="central" {{ old('type') == 'central' ? 'selected' : '' }}>Pusat (Central)</option>
                    <option value="faculty" {{ old('type') == 'faculty' ? 'selected' : '' }}>Fakultas</option>
                    <option value="study_program" {{ old('type') == 'study_program' ? 'selected' : '' }}>Program Studi</option>
                </select>
                @error('type')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div class="mt-6 flex items-center justify-end space-x-3">
            <a href="{{ route('branches.index') }}" class="px-6 py-3 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 transition">
                Batal
            </a>
