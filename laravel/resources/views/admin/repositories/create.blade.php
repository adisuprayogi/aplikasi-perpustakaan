    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl lg:text-2xl font-bold text-gray-900">Tambah Repository</h1>
                    <p class="text-sm text-gray-500 mt-0.5">Submit karya ilmiah ke repository</p>
                </div>
            </div>
        </div>
        <a href="{{ route('repositories.index') }}" class="hidden sm:inline-flex items-center px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100">
    <form method="POST" action="{{ route('repositories.store') }}" enctype="multipart/form-data" class="p-4 lg:p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-4 lg:space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="author_nim" name="author_nim" value="{{ old('author_nim') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 transition"
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="advisor_name" name="advisor_name" value="{{ old('advisor_name') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 transition"
                        ...
                        <div>
                            ...
                            <input type="text" id="co_advisor_name" name="co_advisor_name" value="{{ old('co_advisor_name') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 transition"
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea id="abstract" name="abstract" rows="4"
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 transition"
                        placeholder="Abstrak karya ilmiah...">{{ old('abstract') }}</textarea>
                    @error('abstract')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-4 lg:space-y-6">
                ...
                <div>
                    ...
                    <input type="number" id="year" name="year" value="{{ old('year', date('Y')) }}" required min="1900" max="{{ date('Y') + 1 }}"
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 transition">
                    @error('year')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="program_study" name="program_study" value="{{ old('program_study') }}"
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 transition"
                ...
                <div>
                    ...
                    <select id="access_level" name="access_level" required
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600 transition bg-white">
                        <option value="campus_only" {{ old('access_level') == 'campus_only' ? 'selected' : '' }}>Kampus Saja</option>
                        <option value="public" {{ old('access_level') == 'public' ? 'selected' : '' }}>Publik</option>
                        <option value="registered" {{ old('access_level') == 'registered' ? 'selected' : '' }}>Terdaftar</option>
                        <option value="restricted" {{ old('access_level') == 'restricted' ? 'selected' : '' }}>Terbatas</option>
                    </select>
                    @error('access_level')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="is_downloadable" id="is_downloadable" value="1" {{ old('is_downloadable', 1) ? 'checked' : '' }} class="w-4 h-4 text-purple-600 border-gray-300 rounded focus:ring-purple-600">
        ...
        <div class="mt-6 lg:mt-8 pt-6 lg:pt-8 border-t border-gray-100">
            ...
            <div class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-purple-400 transition">
                <input type="file" name="file" id="file" accept=".pdf,.doc,.docx" required class="hidden" onchange="previewFile(this)">
        ...
        <div class="mt-6 lg:mt-8 flex items-center justify-end gap-3 pt-4 lg:pt-6 border-t border-gray-100">
            <a href="{{ route('repositories.index') }}" class="hidden sm:block px-6 py-3 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 transition font-medium">
                Batal
            </a>
