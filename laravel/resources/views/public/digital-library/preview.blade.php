...
@section('title', 'Preview: ' . $file->title)
...
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            <a href="{{ route('digital-library.show', $file) }}" class="p-2 hover:bg-gray-100 rounded-lg transition">
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    ...
    <div class="preview-container bg-gray-100">
        <iframe src="{{ asset('storage/' . $file->file_path) }}" type="application/pdf"></iframe>
    </div>
    @elseif($file->isImage())
    <!-- Image Preview -->
    <div class="p-6 bg-gray-100 flex items-center justify-center min-h-[500px]">
        <img src="{{ asset('storage/' . $file->file_path) }}" alt="{{ $file->title }}" class="preview-image rounded-lg shadow-lg">
    </div>
    @else
    <!-- Unsupported Preview -->
    <div class="p-12 text-center">
        ...
        <a href="{{ route('digital-library.download', $file) }}" class="inline-flex items-center px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
            </svg>
            Download File
...
<div class="mt-4 bg-white rounded-xl shadow-sm p-5">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p class="text-sm font-semibold text-gray-900">{{ $file->title }}</p>
                <p class="text-xs text-gray-500">{{ $file->file_name }} • {{ $file->file_size_human }}</p>
            </div>
        </div>
        <div class="flex items-center gap-4 text-xs text-gray-500">
            <span class="flex items-center gap-1">
                ...
                {{ number_format($file->view_count) }} dilihat
            </span>
            <span class="flex items-center gap-1">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                </svg>
                {{ number_format($file->download_count) }} diunduh
            </span>
        </div>
    </div>
</div>
